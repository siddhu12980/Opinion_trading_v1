...
const HTTP_SERVER_URL = "https://probo.sidd.live/server/api/v1"
const WS_SERVER_URL = "wss://probo.sidd.live/socket"
...
describe('Trading System Tests', () => {
  ...
  test("Execute minting the opposing selling orders and check WebSocket response", async () => {
    ...
    await axios.post(`${HTTP_SERVER_URL}/onramp/inr`, { userId: seller1Id, amount: 1000000 });
    await axios.post(`${HTTP_SERVER_URL}/onramp/inr`, { userId: seller2Id, amount: 1000000 });
    await axios.post(`${HTTP_SERVER_URL}/onramp/inr`, { userId: seller3Id, amount: 1000000 });
    ...
    expect(seller1StockBalace.data.data[symbol].yes.quantity).toBe(50);
    expect(seller2StockBalance.data.data[symbol].no.quantity).toBe(80);
    expect(seller3StockBalance.data.data[symbol].no.quantity).toBe(60);
    ...
    console.log(seller1InrBalance.data)
    expect(seller1InrBalance.data.data.balance).toBe((1000000 + (sell1Price * quantity1) - (2 * 100 * 1000)));
    expect(seller2InrBalance.data.data.balance).toBe((1000000 + (sell2Price * quantity2) - (2 * 100 * 1000)));
    expect(seller3InrBalance.data.data.balance).toBe(
      (1000000 + (sell3Price * (quantity3 - 10)) - (2 * 100 * 1000))
    );
  });
